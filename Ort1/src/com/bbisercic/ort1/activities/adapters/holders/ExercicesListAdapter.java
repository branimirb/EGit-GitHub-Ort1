...
public class ExercicesListAdapter extends ArrayAdapter<ArticleBean> {

    private static final String TAG = LogUtility.getTag(ExercicesListAdapter.class);

    private int mItemLayoutId;

    private List<ArticleBean> mLoadedExercices;

    private Context mContext;

    public ExercicesListAdapter(Context context, int resource, int textViewResourceId, List<ArticleBean> items) {
        super(context, resource, textViewResourceId, items);
        LogUtility.d(TAG, "Created ExercicesListAdapter");

        mContext = context;
        mItemLayoutId = resource;
        mLoadedExercices = items;
    }
    ...
    private void bindItem(final ViewHolder holder) {
        final ArticleBean lectureBean = mLoadedExercices.get(holder.mPosition);
